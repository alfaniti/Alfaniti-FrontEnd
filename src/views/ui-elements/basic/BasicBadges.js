...
const BasicBadges = () => {
  ...
  const buttonBadges = buttonVariants.map((variant, idx) => (
    <Button key={idx} bg={variant} className="text-capitalize">
      {variant}
      <Badge bg="light" className="ml-1">
        4
      </Badge>
    </Button>
  ));
  ...
  const contextualBadges = buttonVariants.map((variant, idx) => (
    <Badge key={idx} bg={variant} className="mr-1">
      {variant}
    </Badge>
  ));

  const pillBadges = buttonVariants.map((variant, idx) => (
    <Badge key={idx} pill bg={variant} className="mr-1">
      {variant}
    </Badge>
  ));
  ...
  const linkBadges = buttonVariants.map((variant, idx) => (
    <Link to="#" key={idx}>
      <Badge bg={variant} className="mr-1">
        {variant}
      </Badge>
    </Link>
  ));
